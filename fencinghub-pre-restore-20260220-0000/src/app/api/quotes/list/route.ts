...
export async function GET() {
  ...
  const { data: quotes, error } = await admin
    .from("quotes")
    .select(
      "id,status,file_path,project_id,version,pinned,archived,internal_notes,sent_at,viewed_at,responded_at,response_comment,created_at,projects(name,customer_email)",
    )
