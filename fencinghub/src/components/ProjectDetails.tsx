  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data } = await supabase
        .from("projects")
        .select("name,address,materials,preferred_date,notes,internal_notes,created_at,company_id")
        .eq("id", projectId)
        .single();
  ...
  return (
    <div className="card p-4 mb-6">
      ...
      <div className="mt-3 grid gap-3 text-sm sm:grid-cols-2">
        ...
        {!isCustomer && primaryContact && (
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <div className="text-[10px] uppercase tracking-[0.3em] text-[var(--slate-500)]">
              Primary contact
            </div>
            <div className="mt-1 font-medium text-[var(--slate-900)]">
              {primaryContact.full_name || primaryContact.email || "Contact"}
            </div>
            <div className="mt-2 flex flex-wrap gap-2">
              {primaryContact.phone && (
                <a className="btn-ghost" href={`tel:${primaryContact.phone}`}>
                  Call
                </a>
              )}
              {primaryContact.email && (
                <a className="btn-ghost" href={`mailto:${primaryContact.email}`}>
                  Email
                </a>
              )}
            </div>
