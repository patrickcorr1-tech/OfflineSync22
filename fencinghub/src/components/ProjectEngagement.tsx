...
import { getCachedSnags, setCachedSnags } from "@/lib/offlineCache";
...
export default function ProjectEngagement({
  ...
  const load = async () => {
    try {
      ...
      setCachedSnags(projectId, withSnagUrls);
      ...
      const cached = getCachedSnags<any[]>(projectId);
      if (cached?.value) setSnags(cached.value);
    }
  };
  ...
  const createQuoteRequest = async () => {
    ...
    try {
      if (!navigator.onLine) {
        ...
        await enqueueOutbox("quote_request", {
          project_id: projectId,
          title: requestTitle,
  ...
  const updateSnagStatus = async (id: string, status: string) => {
    if (!navigator.onLine) {
      await enqueueOutbox("snag_status", { id, status });
